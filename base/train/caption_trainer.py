...
class CaptionTrainer(MiniBatchTrainer):
    ...
    def load_status(self):
        if self.cfg.checkpoint is not None:
            ...
            self.tracker.epochs_since_improvement = self.checkpoint['epochs_since_improvement']
    ...
    def validate_batch(self, data_dict):
        ...
        print_dict = {'Iter': self.tracker.batch_idx, 'top 5 validation acc': top5,
        'infer time': self.tracker.infer_time, 'load time': self.tracker.load_time}
    ...
    def run_test(self):
        ...
        """
        raise NotImplementedError("Please implement run_test for classes that inherit CaptionTrainer!")
