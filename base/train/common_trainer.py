...
class CommonTrainer(MiniBatchTrainer):
    ...
    def run(self):
        if self.cfg.mode == 'train':
            ...
            if not (self.exp_save_dir / 'model_log').is_dir():
                (self.exp_save_dir / 'model_log').mkdir(parents=True,
                                                        exist_ok=True)
    ...
    def run_epoch(self):
        ...
        if self.cfg.save_by == 'epoch':
            self.save_model(self.tracker.epoch, self.tracker.total_iter,
    ...
    def run_test(self):
        ...
        for batch_idx, data_dict in enumerate(
                self.data_container.dataloader['test']):

            load_time = time() - load_start
            total_test += 1
            test_acc_1, test_acc_5, test_correct, test_num = self.test(
                data_dict)
        ...
        avg_top_5 = total_test_acc_5 / len(
            self.data_container.dataloader['test'])
    ...
    def train_epoch(self):
        ...
        for batch_idx, data_dict in enumerate(
                self.data_container.dataloader['train']):
    ...
    def validate_epoch(self):
        self.tracker.start_load_timing()
        for batch_idx, data_dict in enumerate(
                self.data_container.dataloader['val']):
            self.tracker.set_batch(batch_idx)
            self.tracker.end_load_timing()
